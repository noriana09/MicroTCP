...
struct SentPackets {
  uint32_t seq;        
  uint32_t size;          /*Lista pou apothikeuei ta paketa pou stelnei o client*/
  int num;
...
typedef struct{
  ...
  size_t init_win_size;         /**< The window size negotiated at the 3-way handshake */ /*the bytes that can be sent by one side of the connection before it expects an acknowledgment from the other side*/
