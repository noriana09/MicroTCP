...
int server_microtcp (uint16_t listen_port, const char *file){
    ...
    clock_gettime (CLOCK_MONOTONIC_RAW, &start_time);
    while ((received=microtcp_recv(&sock, buffer, MICROTCP_RECVBUF_LEN, 0))>0) {  /*Lamvanei mexri na erthei to FINACK*/
        fwrite (buffer, sizeof(uint8_t), received, fp);
        total_bytes += received;
        if (received == -1 && sock.state!=CLOSING_BY_PEER) {    /*an lavw -1 kai den einai apto FINACK tote error*/
            perror("microtcp_recv failed.\n");
            break;
        }
        if(sock.state==CLOSING_BY_PEER){  /*FINACK ara exit loop*/
          break;
        }
    }
    if(sock.state==CLOSING_BY_PEER){
			microtcp_shutdown(&sock,0); /*kalw th shutdown afou elava FINACK*/
		}
  clock_gettime (CLOCK_MONOTONIC_RAW, &end_time);
  print_statistics (total_bytes, start_time, end_time);
  free(buffer);
...
int client_microtcp (const char *serverip, uint16_t server_port, const char *file){
    ...
    /*Diavazoume grammh grammh to arxeio kai stelnoume th kathe grammh pou diavazoume*/
    while (fgets(buffer, CHUNK_SIZE, fp) != NULL) {
      size_t len=strlen(buffer);  /*Upologizoume to mhkos tou buffer*/
      /*Elegxw an to buffer einai adeio kai an teleiwnei me new line*/
      if (len > 0 && buffer[len-1] == '\n') {
        /*An teleiwnei me new line tote afairoume to newline kai to antikathistoume me null terminator*/
          buffer[len-1] = '\0';
      }
      /*Apostolh ths grammhs pou diavasthke*/
